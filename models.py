...
class BLSTM_wf_b4_sec(Chain):
    def __init__(self, vocab_size, embed_size, hidden_size, output_size, extra_hidden_size, feat_length):
        super(BLSTM_wf_b4_sec, self).__init__(
            x2e = L.EmbedID(vocab_size, embed_size, ignore_label=-1),
            e2h_for = L.LSTM(embed_size, hidden_size),
            e2h_back = L.LSTM(embed_size, hidden_size),
            h2s = L.Linear(feat_length + (hidden_size*2), extra_hidden_size),
            s2o = L.Linear(extra_hidden_size, output_size)
        )
    ...
    def __call__(self, x, feats):
        ...
        h_states = [F.concat(j) for j in zip(h_states, feats)]
        o_states = [self.s2o(F.relu(self.h2s(h))) for h in h_states]
        return o_states
...
class BLSTM_wf_after_sec(Chain):
    def __init__(self, vocab_size, embed_size, hidden_size, output_size, extra_hidden_size, feat_length):
        super(BLSTM_wf_after_sec, self).__init__(
            x2e = L.EmbedID(vocab_size, embed_size, ignore_label=-1),
            e2h_for = L.LSTM(embed_size, hidden_size),
            e2h_back = L.LSTM(embed_size, hidden_size),
            h2s = L.Linear(hidden_size*2, extra_hidden_size),
            s2o = L.Linear(feat_length + extra_hidden_size, output_size)
        )
    ...
    def __call__(self, x, feats):
        ...
        s_states = [F.relu(self.h2s(h)) for h in h_states]
        s_states = [F.concat(j) for j in zip(s_states, feats)]
        o_states = [self.s2o(s) for s in s_states]
        return o_states
